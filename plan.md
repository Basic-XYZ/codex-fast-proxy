...
- 普通用户主路径固定为：自然语言安装 -> Codex 返回中文 Control UI 链接 -> 用户用外部浏览器打开 -> 点击「启用」-> 自动寻找本机可用端口并完成启用和 ChatGPT 登录准备 -> 提示重启 Codex -> 重启后显示「运行正常」。
- 默认 Control UI 使用中文。
...
- 安装完成后 Control UI 自动寻找可用本地端口，用中文文本提示用户把实际返回的链接放到外部浏览器打开，例如：`请在外部浏览器中打开：http://127.0.0.1:<control-port>/`。
...
- 点击「启用」时数据代理端口也自动寻找可用本地端口；选中的端口必须写入 settings、manifest、Codex provider `base_url` 和后续 hook 读取路径，不能让用户手动理解或选择端口。
