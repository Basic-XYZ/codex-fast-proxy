...
Prefer the Chinese Control UI for normal users:
...
pointing to the local proxy; use Control UI or `set-upstream` to change the saved upstream route.
...
- `set-upstream` and the UI model-service form verify a streaming `POST /v1/responses` route before
  committing settings. This is real provider traffic and may consume a small amount of quota.
- API keys must not be pasted back into chat. Prefer the proxy-managed provider auth file; never
  print API key values, `auth.json` contents, ChatGPT tokens, cookies, request bodies, or prompts.
